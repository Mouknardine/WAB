...
**Contrainte durable — SEO local prioritaire.** Le référencement Lausanne / arc lémanique passe avant les autres arbitrages : JSON-LD complet sur chaque page (Organization, Person, WebPage, FAQPage, BreadcrumbList), `sitemap.xml` avec images légendées, `llms.txt`, `robots.txt`, ping IndexNow (`indexnow.sh`, et le workflow `submit-urls.yml` lançable à la demande depuis l'onglet Actions, qui appelle aussi l'API Bing Webmaster si le secret `BING_WEBMASTER_API_KEY` existe). À maintenir à jour à chaque ajout de page ou d'image. Requêtes cibles : « agence web Lausanne » et « site internet Lausanne » — les deux expressions doivent rester présentes dans le titre, la description, le H1 et la FAQ de l'accueil ; les réponses de la FAQ affichée et celles du bloc FAQPage sont identiques mot pour mot.
